...
%%
%%
%创建RCNN网络  创建网络   %这里的inputSize要与(符号#)行对应起来，确定输入的尺寸大小
inputSize = [224 224 3];  %resnet50
%inputSize = [256 256 3];  %darknet53
% inputSize = [224 224 3];  %resnet18

preprocessedTrainingData = transform(trainingData, @(data)preprocessData(data,inputSize));
numAnchors = 3;
anchorBoxes = estimateAnchorBoxes(preprocessedTrainingData,numAnchors)

%选择resnet50作为检测网络,这里是可以进行调整的(符号#)
% featureExtractionNetwork = resnet50;
% featureLayer = 'activation_40_relu';

%选择darknet53作为检测网络,这里是可以进行调整的
% featureExtractionNetwork = darknet53;
% featureLayer = 'leakyrelu44';

%选择resnet18作为检测网络,这里是可以进行调整的
...
%这里需要修改！！！
%输入 应与以下值之一匹配:
%'alexnet', 'vgg16', 'vgg19', 'resnet18', 'resnet50', 'resnet101', 'googlenet', 'inceptionv3', 'inceptionresnetv2',
%'squeezenet', 'mobilenetv2

fastRCNNLayers = vgg16; 

% lgraph = fastRCNNLayers(inputSize,numClasses,anchorBoxes,featureExtractionNetwork,featureLayer);
%%
%%
%数据增强
...
if doTraining==true
    ...
    [detector, info] = trainFastRCNNObjectDetector(trainingData,fastRCNNLayers,options, ...
        'NegativeOverlapRange',[0 0.3], ...
        'PositiveOverlapRange',[0.6 1]);
else
    % Load pretrained detector for the example.
    pretrained = load('imagenet-googlenet-dag.mat');%这里修改过了！！！
